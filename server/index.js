...
app.post('/api/account/orders', requireAuth, async (req, res, next) => {
  try {
    ...
    const orderItems = items.map(item => {
      ...
      return { productId: product.id, name: product.name, quantity, size, variantId: selection.variant.id, variantName: selection.variant.nameZh, skuId: selection.sku.id, unitPrice: product.salePrice ?? product.price }
    })
