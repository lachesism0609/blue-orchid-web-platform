...
test("filters the catalogue by category, search, price, sale, and stock", () => {
  ...
  assert.deepEqual(
    filterProducts(products, { category: "sale", saleIds: [2, 3] }).map(
      (product) => product.id,
    ),
    [2, 3],
  );
...
test("updates only the selected cart item options", () => {
  ...
  assert.deepEqual(updateCartOptions(cart, 1, { size: "L", colorIndex: 2 }), [
    { productId: 1, size: "S" },
    { productId: 2, size: "L", colorIndex: 2 },
  ]);
});
