...
const copy = {
  ...
  en: { shipping: 'Free shipping on orders over ¥399', language: 'English', search: 'Search', nav: ['New In', 'Women', 'Men', 'Accessories', 'Sale', 'About Us'], collection: 'SUMMER 2026', title: <>Summer essentials<br/>made simple</>, intro: <>Light, comfortable and timeless pieces<br/>for every day.</>, shop: 'Shop now', categories: ['Women', 'Men', 'Bags', 'Shoes', 'Accessories', 'Sale'], popular: 'Popular picks', all: 'View all　→', favourite: 'Add to favourites', services: [['Free shipping', 'On orders over ¥399'], ['Easy returns', '30-day return policy'], ['Secure payment', '100% secure checkout'], ['Customer support', "We're here to help"]] }
}

const productNames = { 1: ['亚麻短袖衬衫', 'Linen short-sleeve shirt'], 2: ['直筒牛仔裤', 'Straight jeans'], 3: ['纯棉基础 T 恤', 'Cotton basic T-shirt'], 4: ['宽松廓形西装外套', 'Oversized blazer'], 5: ['轻盈棉质连衣裙', 'Light cotton dress'], 6: ['极简白色运动鞋', 'Minimal white sneakers'], 7: ['经典皮质托特包', 'Classic leather tote'], 8: ['迷你斜挎包', 'Mini crossbody bag'], 9: ['柔软乐福鞋', 'Soft leather loafers'], 10: ['细带凉鞋', 'Strappy sandals'], 11: ['简约弧形太阳镜', 'Minimal curve sunglasses'], 12: ['真丝方巾', 'Silk square scarf'], 13: ['垂感半身长裙', 'Fluid midi skirt'], 14: ['针织开衫', 'Fine knit cardigan'], 15: ['轻薄风衣', 'Lightweight trench coat'], 16: ['亚麻立领衬衫', 'Linen grandad shirt'], 17: ['锥形休闲长裤', 'Tapered trousers'], 18: ['简约圆领卫衣', 'Minimal crew sweatshirt'], 19: ['轻量夹克', 'Lightweight jacket'], 20: ['编织腋下包', 'Woven shoulder bag'], 21: ['通勤双肩包', 'Commuter backpack'], 22: ['小号手提包', 'Small top-handle bag'], 23: ['尼龙旅行包', 'Nylon travel bag'], 24: ['复古跑鞋', 'Retro runner'], 25: ['方头芭蕾鞋', 'Square-toe ballet flats'], 26: ['真皮短靴', 'Leather ankle boots'], 27: ['精工腕表', 'Classic wristwatch'], 28: ['羊毛渔夫帽', 'Wool bucket hat'], 29: ['细链项链', 'Fine chain necklace'], 30: ['皮质腰带', 'Leather belt'] }
const legacyCategories = { 1: 'women', 2: 'men', 3: 'men', 4: 'women', 5: 'women', 6: 'shoes' }
const saleDiscounts = { 1: 0.8, 2: 0.85, 3: 0.75, 9: 0.8, 10: 0.7, 11: 0.75, 12: 0.8, 13: 0.85, 16: 0.8, 18: 0.75, 25: 0.8, 28: 0.7, 29: 0.8, 30: 0.75 }
const variantImages = {
  women: ['https://images.unsplash.com/photo-1485230895905-ec40ba36b9bc?auto=format&fit=crop&w=750&q=85', 'https://images.unsplash.com/photo-1496747611176-843222e1e57c?auto=format&fit=crop&w=750&q=85'],
  men: ['https://images.unsplash.com/photo-1617137968427-85924c800a22?auto=format&fit=crop&w=750&q=85', 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=750&q=85'],
  bags: ['https://images.unsplash.com/photo-1566150905458-1bf1fc113f0d?auto=format&fit=crop&w=750&q=85', 'https://images.unsplash.com/photo-1594223274512-ad4803739b7c?auto=format&fit=crop&w=750&q=85'],
  shoes: ['https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=750&q=85', 'https://images.unsplash.com/photo-1543163521-1bf539c55dd2?auto=format&fit=crop&w=750&q=85'],
  accessories: ['https://images.unsplash.com/photo-1511499767150-a48a237f0083?auto=format&fit=crop&w=750&q=85', 'https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?auto=format&fit=crop&w=750&q=85']
}
const navigation = [{ id: 'new', zh: '新品', en: 'New in' }, { id: 'women', zh: '女装', en: 'Women' }, { id: 'men', zh: '男装', en: 'Men' }, { id: 'accessories', zh: '配饰', en: 'Accessories' }, { id: 'sale', zh: '特惠', en: 'Sale' }, { id: 'about', zh: '关于我们', en: 'About us' }]
const categoryRoutes = ['women', 'men', 'bags', 'shoes', 'accessories', 'sale']
const categoryImages = ['https://images.unsplash.com/photo-1485968579580-b6d095142e6e?auto=format&fit=crop&w=250&q=80', 'https://images.unsplash.com/photo-1488161628813-04466f872be2?auto=format&fit=crop&w=250&q=80', 'https://images.unsplash.com/photo-1584917865442-de89df76afd3?auto=format&fit=crop&w=250&q=80', 'https://images.unsplash.com/photo-1543163521-1bf539c55dd2?auto=format&fit=crop&w=250&q=80', 'https://images.unsplash.com/photo-1523779917675-b6ed3a42a561?auto=format&fit=crop&w=250&q=80', 'https://images.unsplash.com/photo-1607082349566-187342175e2f?auto=format&fit=crop&w=250&q=80']
const heroSlides = [
...
const Icon = ({ name, size = 21 }) => {
  const paths = { search: <><circle cx="11" cy="11" r="6.5"/><path d="m16 16 4 4"/></>, user: <><circle cx="12" cy="8" r="4"/><path d="M4.5 21c.7-4 3.3-6 7.5-6s6.8 2 7.5 6"/></>, heart: <path d="M20.8 4.8a5.5 5.5 0 0 0-7.8 0L12 5.9l-1.1-1.1a5.5 5.5 0 0 0-7.8 7.8L12 21l8.9-8.4a5.5 5.5 0 0 0-.1-7.8Z"/>, bag: <><path d="M4 8h16l-1 13H5L4 8Z"/><path d="M8 9V6a4 4 0 0 1 8 0v3"/></>, truck: <><path d="M3 6h11v11H3zM14 10h4l3 3v4h-7z"/><circle cx="7" cy="18" r="2"/><circle cx="17" cy="18" r="2"/></>, return: <><path d="M4 8a8 8 0 1 1-1 7"/><path d="M4 3v5h5"/></>, lock: <><rect x="4" y="9" width="16" height="12" rx="1"/><path d="M8 9V6a4 4 0 0 1 8 0v3"/></>, headset: <><path d="M4 14v-2a8 8 0 0 1 16 0v2"/><path d="M4 14h3v5H5a1 1 0 0 1-1-1v-4Zm16 0h-3v5h2a1 1 0 0 0 1-1v-4Z"/></> }
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">{paths[name]}</svg>
}

function BrandLogo({ onHome }) { return <a className="brand" href="#top" onClick={event => { if (onHome) { event.preventDefault(); onHome() } }} aria-label="Blue Orchid home"><svg className="orchid-mark" viewBox="0 0 34 34" aria-hidden="true"><path d="M17 17.2C11.7 16.7 7.4 13.4 8.2 8.6c.6-3.3 4-4.2 6.2-2.1 1.6 1.6 2.2 5.1 2.6 10.7Z"/><path d="M17 17.2c.6-5.2 2.4-9.1 4.6-10.8 2.5-1.9 5.5-.7 5.8 2.4.5 4.6-4.3 7.6-10.4 8.4Z"/><path d="M17 17.2c-2.2 4.8-6.1 6.8-8.7 4.9-2.4-1.8-.7-5.3 2.2-5.8 1.7-.3 4 .2 6.5.9Z"/><path d="M17 17.2c2.5 4.9 6.3 6.8 8.8 4.7 2.1-1.8.6-5-2.3-5.6-1.8-.3-4.1.1-6.5.9Z"/><circle cx="17" cy="17.2" r="2.4"/><path className="stem" d="M17 19.5c0 4.1-1.3 6.6-4.3 8.4"/></svg><span><b>blue</b><em>orchid</em></span></a> }
...
function App() {
  ...
  const [catalogPage, setCatalogPage] = useState(() => navigation.some(item => item.id === window.location.hash.slice(1)) && window.location.hash !== '#about' ? window.location.hash.slice(1) : '')
  const [aboutPage, setAboutPage] = useState(() => window.location.hash === '#about')
  ...
  useEffect(() => { if (accountPage && authUser) loadAccount() }, [accountPage])
  useEffect(() => {
  ...
  const favouriteProducts = products.filter(product => liked.includes(product.id))
  const catalogName = navigation.find(item => item.id === catalogPage)?.[lang] || (lang === 'zh' ? '商品' : 'Products')
  const categoryOf = product => product.category || legacyCategories[product.id]
  ...
  const handleCheckout = async () => {
    if (!authUser) { setAuthError(lang === 'zh' ? '请登录后继续结算。' : 'Please sign in to continue.'); setAuthOpen(true); return }
    try {
      const data = await accountRequest('/api/account/addresses')
      if (!data.addresses.length) { setToast(lang === 'zh' ? '请先在个人主页添加收货地址。' : 'Please add a delivery address first.'); return }
      setCheckoutAddresses(data.addresses)
      setSelectedAddressId(current => data.addresses.some(address => address.id === current) ? current : data.addresses[0].id)
  ...
  return <>
    ...
    {cartPage && <div className="favourites-page checkout-page"><section className="favourites-shell checkout-shell">
      {checkoutStage === 'success' ? <div className="checkout-success"><span>✓</span><p>BLUE ORCHID</p><h1>{lang === 'zh' ? '购买成功' : 'Order confirmed'}</h1><strong>{completedOrder?.id}</strong><p>{lang === 'zh' ? '订单已创建，即将跳转到订单历史。' : 'Your order was created. Opening order history…'}</p></div> : <>
        ...
        {checkoutStage === 'confirm' ? <div className="checkout-review"><section><h2>{lang === 'zh' ? '购买商品' : 'Your items'}</h2><div className="checkout-items">{cartItems.map(item => <article key={item.index}><img src={variantImage(item.product)} alt=""/><div><strong>{productNames[item.productId]?.[lang === 'zh' ? 0 : 1] || item.product.name}</strong><span>{lang === 'zh' ? '数量' : 'Qty'} × {item.quantity}</span></div><b>{formatPrice(cartUnitPrice(item.product) * item.quantity)}</b></article>)}</div><h2>{lang === 'zh' ? '选择收货地址' : 'Delivery address'}</h2><div className="checkout-addresses">{checkoutAddresses.map(address => <label className={selectedAddressId === address.id ? 'selected' : ''} key={address.id}><input type="radio" name="checkout-address" checked={selectedAddressId === address.id} onChange={() => setSelectedAddressId(address.id)}/><strong>{address.recipient}</strong><span>{address.phone}</span><p>{address.line1}<br/>{address.city} {address.postcode}<br/>{address.country}</p></label>)}</div></section><aside><h2>{lang === 'zh' ? '订单汇总' : 'Order summary'}</h2><p><span>{lang === 'zh' ? '商品小计' : 'Subtotal'}</span><strong>{formatPrice(cartTotal)}</strong></p><p><span>{lang === 'zh' ? '配送' : 'Delivery'}</span><strong>{lang === 'zh' ? '免费' : 'Free'}</strong></p><div><span>{lang === 'zh' ? '合计' : 'Total'}</span><strong>{formatPrice(cartTotal)}</strong></div><button className="confirm-order" onClick={confirmOrder} disabled={!selectedAddressId || checkoutLoading}>{checkoutLoading ? (lang === 'zh' ? '正在确认…' : 'Confirming…') : (lang === 'zh' ? '确认购买' : 'Confirm order')}</button><button className="back-to-cart" onClick={() => setCheckoutStage('cart')}>{lang === 'zh' ? '返回购物车' : 'Back to bag'}</button></aside></div> : cartItems.length ? <><div className="cart-list">{cartItems.map(item => <article key={item.index}><img src={variantImage(item.product)} alt=""/><div><strong>{productNames[item.productId]?.[lang === 'zh' ? 0 : 1] || item.product.name}</strong><p>{formatPrice(cartUnitPrice(item.product))}</p></div><div className="quantity-control"><button onClick={() => changeCartQuantity(item.index, -1)}>−</button><span>{item.quantity}</span><button onClick={() => changeCartQuantity(item.index, 1)}>+</button></div><button onClick={() => setCart(current => current.filter((_, index) => index !== item.index))}>{lang === 'zh' ? '移除' : 'Remove'}</button></article>)}</div><div className="cart-summary"><strong>{lang === 'zh' ? '合计' : 'Total'} {formatPrice(cartTotal)}</strong><button onClick={handleCheckout}>{lang === 'zh' ? '去结算' : 'Checkout'}</button></div></> : <div className="catalog-empty"><h2>{lang === 'zh' ? '购物车为空' : 'Your bag is empty'}</h2><button onClick={goHome}>{lang === 'zh' ? '继续选购' : 'Continue shopping'}</button></div>}
      </>}
    </section></div>}
    {favouritesPage && <div className="favourites-page"><section className="favourites-shell"><header className="favourites-heading"><p>BLUE ORCHID</p><h1>{lang === 'zh' ? '我的收藏' : 'My favourites'}</h1><span>{favouriteProducts.length} {lang === 'zh' ? '件商品' : 'items'}</span></header>{favouriteProducts.length ? <div className="catalog-grid">{favouriteProducts.map(product => <article className="card" key={product.id}><div className="product-image"><img src={variantImage(product)} alt={productNames[product.id]?.[lang === 'zh' ? 0 : 1] || product.name}/><button className="liked" onClick={() => toggleLike(product.id)} aria-label={t.favourite}><Icon name="heart" size={20}/></button></div><div className="product-title"><h3>{productNames[product.id]?.[lang === 'zh' ? 0 : 1] || product.name}</h3><button onClick={() => addToCart(product)}>{lang === 'zh' ? '加入购物车' : 'Add to bag'}</button></div>{saleDiscounts[product.id] ? <p className="sale-price"><s>{formatPrice(product.price)}</s><strong>{formatPrice(salePrice(product))}</strong></p> : <p>{formatPrice(product.price)}</p>}<div className="swatches">{product.colors.map((color, index) => <button className={selectedColorIndex(product) === index ? 'selected' : ''} style={{ background: color }} onClick={() => selectColor(product.id, index)} aria-label={`${lang === 'zh' ? '选择颜色' : 'Choose colour'} ${index + 1}`} key={color}/>)}</div></article>)}</div> : <div className="catalog-empty"><h2>{lang === 'zh' ? '还没有收藏商品' : 'No favourites yet'}</h2><p>{lang === 'zh' ? '点击商品右上角的爱心，将心仪单品保存到这里。' : 'Tap the heart on a product to save it here.'}</p><button onClick={goHome}>{lang === 'zh' ? '去逛逛' : 'Explore the store'}</button></div>}</section></div>}
    {catalogPage && <div className="catalog-page"><section className="catalog-shell">{catalogPage === 'about' ? <div className="about-page"><p>BLUE ORCHID</p><h1>{lang === 'zh' ? '为自在而设计' : 'Designed for ease'}</h1><div><p>{lang === 'zh' ? 'Blue Orchid 相信风格应当自然、舒适并且历久弥新。我们精心挑选每一件单品，让日常穿搭更轻松。' : 'Blue Orchid believes style should feel natural, comfortable and enduring. Every piece is considered to make everyday dressing effortless.'}</p><p>{lang === 'zh' ? '从柔软面料到简洁廓形，我们用克制的设计语言，陪伴每一段自由的旅程。' : 'From soft fabrics to clean silhouettes, our understated designs are made for every journey you take.'}</p></div></div> : <><header className="catalog-heading"><p>BLUE ORCHID</p><h1>{catalogName}</h1><span>{catalogProducts.length} {lang === 'zh' ? '件商品' : 'items'}</span></header>{catalogProducts.length ? <div className="catalog-grid">{catalogProducts.map(product => <article className="card" key={product.id}><div className="product-image"><img src={variantImage(product)} alt={productNames[product.id]?.[lang === 'zh' ? 0 : 1] || product.name}/>{saleDiscounts[product.id] && <span className="sale-badge">{saleLabel(product)}</span>}<button className={liked.includes(product.id) ? 'liked' : ''} onClick={() => toggleLike(product.id)} aria-label={t.favourite}><Icon name="heart" size={20}/></button></div><h3>{productNames[product.id]?.[lang === 'zh' ? 0 : 1] || product.name}</h3>{saleDiscounts[product.id] ? <p className="sale-price"><s>{formatPrice(product.price)}</s><strong>{formatPrice(salePrice(product))}</strong></p> : <p>{formatPrice(product.price)}</p>}<div className="swatches">{product.colors.map((color, index) => <button className={selectedColorIndex(product) === index ? 'selected' : ''} style={{ background: color }} onClick={() => selectColor(product.id, index)} aria-label={`${lang === 'zh' ? '选择颜色' : 'Choose colour'} ${index + 1}`} key={color}/>)}</div></article>)}</div> : <div className="catalog-empty"><h2>{lang === 'zh' ? '新品即将上架' : 'New pieces are on their way'}</h2><p>{lang === 'zh' ? '请稍后回来查看本系列。' : 'Please check back soon for this collection.'}</p></div>}</>}</section></div>}
    {aboutPage && <div className="about-brand-page"><article className="about-brand-shell">
      <section className="about-hero"><div><p className="about-eyebrow">BLUE ORCHID · EST. 2026</p><h1>{lang === 'zh' ? <>让日常穿着，<br/>自然地成为风格</> : <>Everyday ease,<br/>distinctive style</>}</h1><p>{lang === 'zh' ? '我们以兰花的从容与韧性为灵感，创作轻盈、舒适且经得起时间考验的衣橱单品。' : 'Inspired by the quiet strength of the orchid, we create light, comfortable pieces designed to endure.'}</p><a href="mailto:service@blueorchid.com">{lang === 'zh' ? '联系我们' : 'Contact us'}　→</a></div><img src="https://images.unsplash.com/photo-1523381210434-271e8be1f52b?auto=format&fit=crop&w=1200&q=88" alt={lang === 'zh' ? 'Blue Orchid 服装陈列' : 'Blue Orchid clothing collection'}/></section>
      <section className="about-story"><img src="https://images.unsplash.com/photo-1490481651871-ab68de25d43d?auto=format&fit=crop&w=900&q=88" alt={lang === 'zh' ? '品牌服装设计' : 'Fashion design'}/><div><p className="about-eyebrow">{lang === 'zh' ? '我们的故事' : 'OUR STORY'}</p><h2>{lang === 'zh' ? '少一点喧哗，多一点真正的舒适' : 'Less noise, more considered comfort'}</h2><p>{lang === 'zh' ? 'Blue Orchid 始于一个简单的想法：好衣服不该让人费力。我们从面料触感、剪裁比例和真实生活出发，让每件衣服都能轻松融入日常。' : 'Blue Orchid began with a simple belief: good clothes should never feel difficult. We start with touch, proportion and real life, making every piece effortless to wear.'}</p><p>{lang === 'zh' ? '我们偏爱克制的色彩、清晰的廓形和能够反复搭配的设计，也认真对待每一道制作细节。' : 'We favour restrained colour, clean silhouettes and thoughtful details that work together season after season.'}</p></div></section>
      <section className="about-values"><header><p className="about-eyebrow">{lang === 'zh' ? '我们在意的事' : 'WHAT MATTERS TO US'}</p><h2>{lang === 'zh' ? '从一件衣服，开始更好的日常' : 'A better everyday starts with one good piece'}</h2></header><div><article><span>01</span><h3>{lang === 'zh' ? '舒适面料' : 'Comfort first'}</h3><p>{lang === 'zh' ? '优先选择亲肤、透气和适合长时间穿着的材料。' : 'Soft, breathable materials selected for all-day wear.'}</p></article><article><span>02</span><h3>{lang === 'zh' ? '长久设计' : 'Made to last'}</h3><p>{lang === 'zh' ? '不过度追逐潮流，让剪裁与配色经得起时间。' : 'Timeless cuts and colours that move beyond short-lived trends.'}</p></article><article><span>03</span><h3>{lang === 'zh' ? '贴心服务' : 'Human service'}</h3><p>{lang === 'zh' ? '从选购到退换，用清晰、友好的方式回应每个问题。' : 'Clear, friendly support from first choice to easy returns.'}</p></article></div></section>
      <section className="about-contact"><div className="about-contact-image"><img src="https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=1000&q=88" alt={lang === 'zh' ? 'Blue Orchid 门店' : 'Blue Orchid studio'}/></div><div><p className="about-eyebrow">{lang === 'zh' ? '客户服务' : 'CUSTOMER CARE'}</p><h2>{lang === 'zh' ? '我们很乐意听到你的声音' : "We'd love to hear from you"}</h2><p>{lang === 'zh' ? '关于尺码、商品、订单或退换货，如有任何疑问，请联系我们的客服团队。' : 'For sizing, products, orders or returns, our customer care team is ready to help.'}</p><dl><div><dt>{lang === 'zh' ? '客服邮箱' : 'Email'}</dt><dd><a href="mailto:service@blueorchid.com">service@blueorchid.com</a></dd></div><div><dt>{lang === 'zh' ? '服务热线' : 'Phone'}</dt><dd><a href="tel:+864008002026">+86 400 800 2026</a></dd></div><div><dt>{lang === 'zh' ? '服务时间' : 'Hours'}</dt><dd>{lang === 'zh' ? '周一至周日 09:00–21:00' : 'Mon–Sun, 09:00–21:00'}</dd></div></dl><div className="about-contact-actions"><a href="mailto:service@blueorchid.com">{lang === 'zh' ? '发送邮件' : 'Send email'}</a><a href="tel:+864008002026">{lang === 'zh' ? '致电客服' : 'Call us'}</a></div></div></section>
    </article></div>}
